...
- Type annotations across all test files (`tests/test_game_details.py`, `tests/test_bgg_collection.py`, etc.) for
  improved developer experience and catch potential test-logic bugs
...
- Database schema: `Game.complexity` is now optional (`float | None`) to gracefully handle BGG accessories/promos that
  lack weight ratings
- Updated `open_modal` to correctly display "Weight: N/A" for games without a complexity rating and properly refresh
  the SQLAlchemy session for newly fetched games
